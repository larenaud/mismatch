...
neonatal_dates <- read.csv("data/mine/neonatal_dates.csv")
head(neonatal_dates)
...
# here we define annual population mismatch (PM) as the arithmetic average of IM values each year
dt <- surv_bdate %>%
...
# saving fitness data as R object -----------------------------------------
...
mean(dt1$evi_pm.r) # OK now =probably caused by regressing over duplicated years 
mean(dt1$evi_pm)
mean(dt1$evi_im)
mean(dt1$evi_im.r)
...
View(t1<-table(dt1$mom_id)) #  1 - 12 captures 
dt1$mom_id<- droplevels(dt1$mom_id) # n = 69 females
